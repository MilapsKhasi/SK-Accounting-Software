  ...
  container.innerHTML = `
    <div class="p-8 space-y-8">
      <header class="flex justify-between items-end">
        <div>
          <h1 class="text-3xl font-medium text-gray-900 tracking-tight">Sales Invoices</h1>
          <p class="text-gray-500 mt-1">Manage your sales entries and invoices</p>
        </div>
        <button id="new-sales-btn" class="flex items-center gap-2 px-4 py-2 bg-[#1e2a38] text-white hover:bg-[#2c3e50] transition-colors text-sm font-medium uppercase tracking-widest">
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-gray-50/50 border-b border-gray-100">
                <th class="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-widest">Invoice No</th>
                <th class="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-widest">Date</th>
                <th class="px-6 py-4 text-xs font-medium text-gray-400 uppercase tracking-widest">Customer</th>
            ...
            <tbody class="divide-y divide-gray-50">
              ${sales.map(entry => `
                <tr class="hover:bg-gray-50/50 transition-colors cursor-pointer group" data-id="${entry.id}">
                  <td class="px-6 py-4 font-mono text-sm font-medium text-gray-900">${entry.invoiceNumber}</td>
                  <td class="px-6 py-4 text-sm text-gray-500">${entry.date}</td>
                  <td class="px-6 py-4 text-sm font-medium text-gray-900">
                    <button class="hover:text-[#1e2a38] hover:underline text-left font-medium" onclick="event.stopPropagation(); const c = state.company.modules.customers.find(cust => cust.name === '${entry.customer}'); if(c) window.navigateToLedger('customer', c.id)">
                      ${entry.customer}
                    </button>
                  </td>
                  ...
                  <td class="px-6 py-4">
                    <span class="px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider ${
    ...
    modal.innerHTML = `
      <div class="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-gray-200">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
          <h2 class="text-xl font-medium text-gray-900 uppercase tracking-tight">${isEditing ? 'Edit' : 'New'} Sales Entry</h2>
        ...
        <form id="sales-form" class="p-8 space-y-8">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="space-y-2">
              ...
              <input name="invoiceNumber" value="${initialData.invoiceNumber}" class="w-full px-4 py-2 border border-gray-200 focus:ring-1 focus:ring-[#1e2a38] outline-none transition-all font-medium" />
            </div>
            <div class="space-y-2">
            ...
            <div class="space-y-2">
              ...
              <input name="customer" value="${initialData.customer}" list="customers-list" class="w-full px-4 py-2 border border-gray-200 focus:ring-1 focus:ring-[#1e2a38] outline-none transition-all font-medium" required />
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-8 py-2.5 bg-[#1e2a38] text-white text-xs font-medium hover:bg-[#2c3e50] transition-all uppercase tracking-widest">Save Invoice</button>
          </div>
        </form>
      </div>
    `;
