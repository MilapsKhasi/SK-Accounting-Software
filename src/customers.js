  ...
  container.innerHTML = `
    <div class="p-8 space-y-8">
      <header class="flex justify-between items-end">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Customer Master</h1>
          <p class="text-gray-500 mt-1">Manage your clients and customers</p>
        </div>
        <button id="new-customer-btn" class="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm text-sm font-medium">
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-gray-50/50 border-b border-gray-100">
                <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Customer Name</th>
            ...
            <tbody class="divide-y divide-gray-50">
              ${customers.map(customer => `
                <tr class="hover:bg-gray-50/50 transition-colors cursor-pointer group" data-id="${customer.id}">
                  <td class="px-6 py-4 text-sm font-bold text-gray-900">${customer.name}</td>
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button class="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="View Ledger" onclick="event.stopPropagation(); window.navigateToLedger('customer', '${customer.id}')">
                        <i data-lucide="book-open" class="w-4 h-4"></i>
                      </button>
                      <button class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" onclick="event.stopPropagation(); window.deleteCustomer('${customer.id}')">
  ...
  modal.innerHTML = `
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-100">
      <div class="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
        <h2 class="text-xl font-bold text-gray-900">${isEditing ? 'Edit' : 'Add'} Customer</h2>
      ...
      <form id="customer-form" class="p-8 space-y-6">
        <div class="space-y-2">
          <label class="text-xs font-bold text-gray-400 uppercase tracking-widest">Customer Name</label>
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" class="px-8 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all">Save Customer</button>
        </div>
      </form>
    </div>
  `;
