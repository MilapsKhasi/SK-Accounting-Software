...
import { Vendor, PurchaseEntry } from '../types';
import { 
  Users, 
  Plus, 
  Trash2, 
...
export const VendorMasterModule: React.FC = () => {
  const { company, saveVendor, deleteVendor } = useAccounting();
  ...
  const exportToPDF = (vendor: Vendor) => {
    ...
    const netBalance = Math.abs(totalCredit - totalDebit);
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text(vendor.name.toUpperCase(), 105, 38, { align: 'center' });

    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(vendor.gstin || "VENDOR'S GSTIN NUMBER", 105, 44, { align: 'center' });
    ...
    doc.save(`${vendor.name}_Ledger_${financialYear}.pdf`);
    setShowExportDropdown(false);
  };

  const getLedgerData = (vendor: Vendor) => {
    const purchases = company.modules.purchase.filter(p => p.vendor === vendor.name);
    const ledger: any[] = [];

    purchases.forEach(p => {
      // Add Bill row (Credit for Vendor)
      ledger.push({
        date: p.date,
      ...
      // Add Payment rows (Debit for Vendor)
      if (p.payments) {
        p.payments.forEach(pay => {
          ledger.push({
            date: pay.date,
            particulars: `Payment Made for Purchase Bill - ${p.billNumber} (${pay.method})`,
            debit: pay.amount,
            credit: 0,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const vendorData: Vendor = {
      ...
      ...formData,
      createdAt: editingId 
        ? company.modules.vendors.find(v => v.id === editingId)?.createdAt || new Date().toISOString().split('T')[0]
  ...
  const getVendorTransactions = (vendorName: string) => {
    return company.modules.purchase.filter(p => p.vendor.toLowerCase() === vendorName.toLowerCase());
  };

  if (selectedVendor) {
    const ledgerData = getLedgerData(selectedVendor);
    const totalDebit = ledgerData.reduce((sum, r) => sum + r.debit, 0);
    const totalCredit = ledgerData.reduce((sum, r) => sum + r.credit, 0);
    const netBalance = Math.abs(totalCredit - totalDebit);
    ...
    return (
      <div className="p-8 space-y-8 max-w-5xl mx-auto bg-white shadow-sm border border-gray-100 rounded-3xl">
        ...
        <div className="flex flex-col items-center text-center py-12">
          ...
          <div className="mb-8">
            <h3 className="text-xl font-black text-gray-900 uppercase tracking-widest mb-1">{selectedVendor.name}</h3>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{selectedVendor.gstin || "VENDOR'S GSTIN NUMBER"}</p>
          </div>
          ...
          <form onSubmit={handleSave} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden sticky top-24">
            <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex justify-between items-center">
              <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2">
                <Plus className="w-4 h-4 text-indigo-600" /> {editingId ? 'Edit Vendor' : 'Add New Vendor'}
              </h2>
              {editingId && (
                <button 
                  type="button"
            ...
            <div className="p-6 space-y-4">
              ...
              >
                <Save className="w-4 h-4" /> {editingId ? 'Update Vendor' : 'Save Vendor'}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-50 flex justify-between items-center">
              <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Vendor Directory</h2>
              <span className="text-xs text-gray-400 font-medium">{company.modules.vendors.length} Vendors</span>
            </div>
            <div className="divide-y divide-gray-50">
              <AnimatePresence>
                {company.modules.vendors.map((vendor) => (
                  <motion.div 
                    key={vendor.id}
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    ...
                    className="p-6 hover:bg-indigo-50/30 transition-colors group cursor-pointer"
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500 flex items-center gap-1.5 mt-0.5">
                            <span className="font-medium text-gray-700">{vendor.contactPerson || 'No Contact Person'}</span>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity">View History</span>
                        <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-600 md:col-span-3">
                        ...
                        <span className="line-clamp-1">{vendor.address || 'No Address Provided'}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
              {company.modules.vendors.length === 0 && (
