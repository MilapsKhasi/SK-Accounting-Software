...
export const PurchaseEntryModule: React.FC = () => {
  const { company, ensureVendorExists, savePurchaseEntry, cancelPurchaseEntry, restorePurchaseEntry } = useAccounting();
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    if (!formData.billNumber || !formData.vendor) return;

    // Check for duplicates
    const isDuplicate = company.modules.purchase.some(p => 
      p.billNumber.toLowerCase() === formData.billNumber.toLowerCase() && p.id !== editingId
    );

    if (isDuplicate) {
      setError(`Bill number "${formData.billNumber}" already exists!`);
      return;
    }
  ...
  const cancelEntry = async (id: string) => {
    const entry = company.modules.purchase.find(e => e.id === id);
    if (entry) {
  ...
  const handleUndo = async () => {
    if (undoAction) {
      await restorePurchaseEntry(undoAction.id, undoAction.previousStatus);
      setUndoAction(null);
    }
  };
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
          ...
          <form onSubmit={handleSave} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex justify-between items-center">
              <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2">
                <Receipt className="w-4 h-4 text-indigo-600" /> {editingId ? 'Edit Bill' : 'Bill Details'}
              </h2>
              {editingId && (
                <button 
                  type="button"
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2 space-y-1.5">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Vendor / Supplier</label>
                <input 
                  type="text" 
                  name="vendor"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:col-span-2">
                ...
                <div className="flex items-end">
                  ...
                  >
                    <Save className="w-4 h-4" /> {editingId ? 'Update Bill' : 'Save Bill'}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-50 bg-gray-50/30">
              <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Bill Summary</h2>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="pt-4">
                <div className="bg-indigo-600 p-4 rounded-xl text-white shadow-lg shadow-indigo-100">
                  <p className="text-xs font-bold uppercase tracking-widest opacity-80">Net Bill Amount</p>
              ...
              <div className="p-6 space-y-6">
                <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Total Bill Amount</span>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex gap-3">
                ...
                <button 
                  ...
                  onClick={() => {
                    ...
                    if (totalPaid >= calculated.netBill - 0.01) {
                      setStatus('Paid');
                    } else if (totalPaid > 0) {
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center">
          ...
          <span className="text-xs text-gray-400 font-medium">{company.modules.purchase.filter(b => b.status !== 'Canceled').length} Transactions</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="p-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-100">Bill No</th>
                <th className="p-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-100">Date</th>
                <th className="p-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-100">Vendor</th>
            ...
            <tbody className="divide-y divide-gray-50">
              <AnimatePresence>
                {company.modules.purchase.filter(b => b.status !== 'Canceled').map((bill) => (
                  <motion.tr 
                    key={bill.id}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                    <td className="p-4 text-sm font-bold text-gray-900">{bill.billNumber}</td>
                    <td className="p-4 text-sm text-gray-500 font-mono">{bill.date}</td>
                    <td className="p-4 text-sm text-gray-700 font-medium">{bill.vendor}</td>
                    <td className="p-4 text-sm text-gray-500">{bill.itemName}</td>
                    <td className="p-4 text-sm text-gray-900 font-mono text-right">₹{bill.amount.toFixed(2)}</td>
                    <td className="p-4 text-sm text-gray-500 font-mono text-right">
                      {bill.gstEnabled ? `₹${bill.gstAmount.toFixed(2)}` : '-'}
                    </td>
                    <td className="p-4 text-sm font-bold text-indigo-600 font-mono text-right">₹{bill.netBill.toFixed(2)}</td>
                    <td className="p-4">
              ...
              {company.modules.purchase.filter(b => b.status !== 'Canceled').length === 0 && (
                <tr>
